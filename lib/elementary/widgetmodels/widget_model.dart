...
import 'package:testtask/ui/viewcontroller/slider_controller.dart';
import 'package:testtask/ui/viewmodel/shape_model.dart';
import 'package:testtask/ui/viewmodel/sliders_colors.dart';
...
class GenerateImageWidgetModel extends WidgetModel<GenerateImage, GenerateImageModel> {
  ...
  @override
  ValueListenable<EntityState<String>> get textState => _textController;

  late EntityStateNotifier<String> _textController;
  ...
  @override
  void changeTetx(String factor) {
    _textController.loading();

    final newVal = model.setText(factor);

    _textController.content(newVal);
  }

  @override
  void initWidgetModel() {
    super.initWidgetModel();
    unawaited(generateStartShape());
    ...
    _textController = EntityStateNotifier<String>.value(model.text);
  }
 ...
 /* void setColor(double factor) {
  ...
  Future<void> generateStartShape() async {
    _isLoadingShapeController.add(true);
    final newShapeDto = await model.getStartShape();
    _shapeController.add(ShapeMetrics.fromDto(newShapeDto));
    widthController.add(newShapeDto.width);
    heightController.add(newShapeDto.height);
    opacityController.add(newShapeDto.opacity);
